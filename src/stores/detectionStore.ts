...
interface DetectionStoreState {
  ...
  getDetectionById: (clusterId: string) => DetectionData | undefined;
  filterDetections: (filters: { type?: string; uav?: string }) => void;
}
...
const initialDetectionData: DetectionData[] = [
  generateClusterData({
...
export const useDetectionStore = create<DetectionStoreState>((set, get) => ({
  detections: initialDetectionData,
  filteredDetections: initialDetectionData,
  selectedDetection: null,

  setFilteredDetections: (detections: DetectionData[]) => set({ filteredDetections: detections }),

  setSelectedDetection: (detection: DetectionData | null) => set({ selectedDetection: detection }),

  getDetectionById: (clusterId: string) => {
    const { detections } = get();
    return detections.find(detection => detection.clusterId === clusterId);
  },

  filterDetections: (filters: { type?: string; uav?: string }) => {
    const { detections } = get();
    let filtered = detections;

    if (filters.type) {
      filtered = filtered.filter((detection) => detection.objectsType === filters.type);
    }

    if (filters.uav) {
      filtered = filtered.filter((detection) =>
        detection.responsibleUAV.toLowerCase().includes(filters.uav!.toLowerCase())
      );
    }

    set({ filteredDetections: filtered });
  },
}));
